...
struct TileStoreOpConversion : public OpRewritePattern<arm_sme::TileStoreOp> {
  ...
  LogicalResult matchAndRewrite(arm_sme::TileStoreOp tileStoreOp,
                                PatternRewriter &rewriter) const override {
    if (tileStoreOp.getMask()) {
      auto createMaskOp = tileStoreOp.getMask().getDefiningOp<vector::CreateMaskOp>();
      if (!createMaskOp)
        return rewriter.notifyMatchFailure(
            tileStoreOp.getLoc(), "unsupported mask op, only 'vector.create_mask' is "
                "currently supported");
    }
